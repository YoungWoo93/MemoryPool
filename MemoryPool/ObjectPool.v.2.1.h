...
public:
		...
		: maxSize(0), useSize(_size), top(0), linkPtr(nullptr), useCount(0), heap(HeapCreate(0, 0, 0))
	{
		for (int i = 0; i < _size; i++) {
			memoryBlock<T>* newNode = newMemoryBlock();
	...
	T* Alloc()
	{
		...
		retNode->next = (memoryBlock<T>*)this;
...
private:
	...
	memoryBlock<T>* newMemoryBlock()
	{
		memoryBlock<T>* newNode = (memoryBlock<T>*)HeapAlloc(heap, 0, sizeof(memoryBlock<T>));// new memoryBlock<T>();
...
public:
	...
	template <typename... param>
	AlignObjectPool(int _size)
		: maxSize(0), useSize(_size), top(0), linkPtr(nullptr), useCount(0), heap(HeapCreate(0, 0, 0))
	{
		for (int i = 0; i < _size; i++) {
			alignMemoryBlock<T>* newNode = newMemoryBlock();
	...
	T* Alloc()
	{
		...
		alignMemoryBlock<T>* retNode = (alignMemoryBlock<T>*)MAKE_NODEPTR(ret);
		retNode->next = (alignMemoryBlock<T>*)this;
	...
	T* New(param... constructorArgs)
	{
		...
		alignMemoryBlock<T>* newNode = (alignMemoryBlock<T>*)MAKE_NODEPTR(ret);
		newNode->next = (alignMemoryBlock<T>*)this;
		newNode->data = new (newNode->data) T(constructorArgs...);
...
private:
	...
	// 혹은 next에 this를 연결할지 (= 할당 후 바로 메모리풀 외부로 반환할지)
	// 는 newMemoryBlock 함수를 사용하는 곳에서 결정 후 동작
	alignMemoryBlock<T>* newMemoryBlock()
	{
		int cashSize = 64;
		...
		//alignMemoryBlock<T>* newNode = (alignMemoryBlock<T>*)constructPtr;

		alignMemoryBlock<T>* newNode = new (constructPtr) alignMemoryBlock<T>(allocPtr);
		//newNode->next = top;
		//newNode->freePtr = (alignMemoryBlock<T>*)((PTRSIZEINT)FreePtr | CHECK),
